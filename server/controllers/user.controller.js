...
const register = async (req, res, next) => {
    try {
        ...
        const user = await User.create({
            ...
            password,
            avatar: {
                public_id: email, // ✅ this is fine
                secure_url: 'https://res.cloudinary.com/du9jzqlpt/image/upload/v1674647316/avatar_drzgxv.jpg',
            }
        });

        // ✅ Debug log to check file upload details
        ...
        if (req.file) {
            try {
                ...
                return next(new AppError(`File upload failed: ${e.message}`, 500));
            }
        }

        await user.save(); // ✅ Ensures updated avatar gets saved
        user.password = undefined;

        // ✅ Generate JWT Token (use await as it's an async method)
        ...
        console.error("Registration Error:", e); // ✅ Log error for debugging
        return next(new AppError(`Registration failed: ${e.message}`, 500));
    }
};
